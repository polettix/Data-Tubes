...
in I<scalar> context, different things are returned depending on what
the C<$tube> returns. If it returns a single item (i.e. a record), it is
returned back. If it returns the string C<records> and an array
