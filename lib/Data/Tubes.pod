...
This document describes Data::Tubes version {{[={local*STDOUT;open STDOUT,'>&',V 'STDOUT';V('dzil')->version()}]}}.
...
an iterator (like a C<sequence> in L<Data::Tubes::Plugin::Plumbing>)
...
C<sequence> in L<Data::Tubes::Plugin::Plumbing>.

The C<@tubes> are passed to C<sequence> (see
L<Data::Tubes::Plugin::Plumbing>) as parameter C<tubes>. Basically,
...
set to either the string C<sink> or to a subroutine ref. In the first
...
Note that this C<tap> will always be provided with an iterator, which
...
it a bit easier, by means of C<resolve_module> in L<Data::Tubes::Util>.
In particular:
...
The first parameter is used as a I<locator> of a factory method to
...
that package. The function C<load_sub> in L<Data::Tubes::Util> is used
to load the factory method automatically, which means that the package
name is subject to the same rules described in L</summon> above.
