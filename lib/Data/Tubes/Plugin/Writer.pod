...
override parameter C<filename> (for obvious reasons!), plus options
from C<dispatch> in L<Data::Tubes::Plugin::Plumbing>, plus the following
ones:
...
The central argument is C<filename>. You can set it in different ways:
...
data to be inserted as footer when closing/releasing a file. See
L</Data::Tubes::Util::Output>;

=item C<header>

data to be inserted as header when opening/starting to use a file. See
L</Data::Tubes::Util::Output>;
...
data to be inserted between records printed out. See
L</Data::Tubes::Util::Output>;
